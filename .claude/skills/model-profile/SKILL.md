...
allowed-tools: Read, Write, Bash(node .harness/scripts/bench-runner.mjs:*), Bash(node .harness/scripts/bench-compare.mjs:*), Bash(node .harness/scripts/regression-runner.mjs:*), Bash(node .harness/scripts/regression-compare.mjs:*)
---
...
2. Pick the smallest representative task set that exercises that lane.
3. Run baseline and candidate profiles with the same tasks, prompts, and transport.
4. Compare:
   - pass rate
   - hidden check result
   - latency
...
5. Recommend a model only for the lane that was measured.
