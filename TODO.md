...
- [x] **P0-缺陷｜inputElement 的 OS 键盘输入依赖窗口焦点** —— **已修（verifyEffect 声明式回读校验）**：OS 键入后 runner 回读目标元素 value 比对，焦点被抢静默失败会变成显式步骤失败；DOM 路径内建回读；getText 支持表单 value 回退。小红书流程已改 `simulateKeyboard:false`（DOM 合成）+ 前置 clickElement 聚焦。
