"""
Emitter handler definitions — container/structural commands.
These are expanded by the Python emitter, not executed by a handler.
"""
from src.runtime.workflow.handlers.registry import register_handler
@register_handler(cmd="try", label="尝试执行", category="异常处理", runtime="control",
    is_container=True, closes_with="endTry",
    icon="fa-shield-alt", icon_color="text-red-500", bg_color="bg-red-50", category_order=48, command_order=10,
    description="尝试执行内部指令，出错时跳到 catch 分支")
class TryHandler:
    params = []
