...
def build_python(wf: models.Workflow, flat_nodes: list[models.WorkflowNode], repo_root: str = "", initial_params: dict | None = None) -> list[str]:
    ...
    # Resolve workflow-level parameter defaults / runtime overrides
    param_values = {}
    wf_parameters = []
    try:
        wf_parameters = json.loads(wf.parameters or "[]") if hasattr(wf, "parameters") else []
    except Exception:
        wf_parameters = []
    for p in wf_parameters:
        name = p.get("name")
        if not name:
            continue
    ...
    if initial_params:
        param_values.update(initial_params)
    ...
    if wf.url:
        lines.append(f"tab.get(_resolve_vars({repr(wf.url)}))")
    lines.append("")
