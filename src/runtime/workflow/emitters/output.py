...
def _emit_pushItem(node, extra, depth, prefix, by_parent, lines, element_map=None):
    ...
    try:
        ...
        expr_str = json.dumps(parsed, ensure_ascii=False, indent=None)
    except Exception:
        expr_str = expr
...
def _emit_takeScreenshot(node, extra, depth, prefix, by_parent, lines, element_map=None):
    ...
    if loc:
        lines.append(f"{prefix}tab.ele({_py_str(loc)}).get_screenshot(path={_py_str(path)})")
    elif full:
        lines.append(f"{prefix}tab.get_screenshot(path={_py_str(path)}, full_page=True)")
    else:
        lines.append(f"{prefix}tab.get_screenshot(path={_py_str(path)})")
...
def _emit_saveToFile(node, extra, depth, prefix, by_parent, lines, element_map=None):
    ...
    if fmt == "json":
        ...
        lines.append(f"{prefix}with open({_py_str(path)}, 'w', encoding='utf-8') as f:")
        lines.append(f"{prefix}    json.dump({data_var}, f, ensure_ascii=False, indent=2)")
    else:
        lines.append(f"{prefix}import csv")
        lines.append(f"{prefix}with open({_py_str(path)}, 'w', newline='', encoding='utf-8') as f:")
