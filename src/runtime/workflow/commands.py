    ...
    "forEachElement": {
        ...
        "fields": [
            ...
            _scope_field(),
            {
                ...
                "type": "select",
                "options": [
                    {"label": "既渲染又在视口内", "value": "visible"},
                    {"label": "已渲染（可在屏幕外）", "value": "rendered"},
                    {"label": "在视口内（不检查渲染状态）", "value": "viewport"},
                    {"label": "全部（含隐藏元素）", "value": "any"},
                ],
                "default": "visible",
                "group": "advanced",
                "description": "四种过滤组合：全部=任意DOM；已渲染=排除display:none/visibility:hidden等；在视口内=排除屏幕外元素；既渲染又在视口内=同时满足前两者的交集。",
            },
