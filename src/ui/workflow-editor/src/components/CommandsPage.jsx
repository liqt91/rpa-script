...
export default function CommandsPage() {
  ...
  return(<div className="p-6">
    <div className="mb-6 flex items-center justify-between">
      <div><h1 className="text-xl font-semibold text-body">指令管理</h1><p className="text-muted text-sm mt-1">共 {categories.length} 个分类，{Object.values(allCommands).reduce((s,a)=>s+a.length,0)} 个指令，{containerTypes.length} 种容器</p></div>
      <div className="flex items-center gap-2">
        <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索类型/标签/说明..." className="px-3 py-1.5 bg-surface-2 border border-border-strong rounded text-sm text-body placeholder-gray-500 outline-none focus:border-blue-500 w-56"/>
        <button onClick={loadCommands} className="px-3 py-1.5 bg-accent hover:bg-accent-strong text-body rounded text-sm flex items-center gap-2 transition-colors"><i className="fas fa-sync-alt"></i>刷新</button>
      </div>
    </div>
    ...
    {filtered.length===0?(<div className="text-center py-12 text-muted">无匹配指令</div>):(<div className="space-y-3">
      {filtered.map(({cat,cmds})=>(<div key={cat} className="bg-surface-2 rounded-xl border border-border overflow-hidden">
        <div className="px-4 py-2.5 bg-surface-3 border-b border-border flex items-center gap-3"><span className="text-sm font-medium text-muted">{cat}</span><span className="text-xs text-muted">{cmds.length} 个</span></div>
        <div className="overflow-x-auto"><table className="w-full text-xs table-fixed">
          ...
          <tbody>{cmds.map(cmd=>{
            ...
            return(<tr key={cmd.cmd} className="border-b border-border/30 hover:bg-surface-3 cursor-pointer" onClick={()=>setEditCmd({...type,fields:JSON.parse(JSON.stringify(cmd.fields||[]))})}>
      ...
      <div className="bg-surface-2 rounded-xl border border-border w-[740px] max-w-[95vw] max-h-[90vh] flex flex-col" onClick={e=>e.stopPropagation()}>
        <div className="px-5 py-3 border-b border-border flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <h3 className="text-base font-semibold text-body font-mono">{editCmd.cmd}</h3>
          ...
          <button onClick={()=>setEditCmd(null)} className="text-faint hover:text-body"><i className="fas fa-times"></i></button>
        </div>
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
            ...
            {viewSource && (
              <div className="border border-border/50 rounded-lg overflow-hidden">
                <div className="flex items-center justify-between px-3 py-2 bg-bg/80 border-b border-border/50">
                  ...
                  <button onClick={() => setViewSource(null)} className="text-faint hover:text-body"><i className="fas fa-times"></i></button>
                </div>
                {viewSource.loading ? (
                  ...
                  <pre className="p-4 text-[11px] font-mono text-muted bg-surface-3 overflow-auto max-h-64 leading-relaxed">{viewSource.source}</pre>
                ) : (
          ...
          <div>
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">{editCmd.fields.map((f,i)=>(<Fragment key={i}>
              ...
              <div className="w-[74px] flex items-center gap-0.5"><span className="text-[10px] text-muted shrink-0">🔒</span><input value={f.name||''} disabled className="flex-1 px-1.5 py-1 bg-surface-3/50 border border-border rounded text-faint text-[10px] font-mono" placeholder="url"/></div>
              <input value={f.label||''} onChange={e=>{const fs=[...editCmd.fields];fs[i]={...fs[i],label:e.target.value};setEditCmd({...editCmd,fields:fs});}} placeholder="显示名" className="w-[65px] px-1.5 py-1 bg-transparent border border-border-strong rounded text-body text-[10px] outline-none focus:border-blue-500"/>
              <div className="w-[74px] px-1 py-1 bg-surface-3/50 border border-border rounded text-faint text-[10px] flex items-center gap-0.5"><span className="text-[10px] text-muted">🔒</span><span className="truncate">{(FIELD_TYPES.find(t=>t.v===f.type)||{}).l||f.type||'text'}</span></div>
              <select value={f.group||'主属性'} onChange={e=>{const fs=[...editCmd.fields];fs[i]={...fs[i],group:e.target.value};setEditCmd({...editCmd,fields:fs});}} className="w-[58px] px-1 py-1 bg-surface-2 border border-gray-500 rounded text-body text-[10px] outline-none focus:border-blue-500">{FIELD_GROUPS.map(g=><option key={g.v} value={g.v}>{g.l}</option>)}</select>
            ...
            {showTypeHelp && (
              <div className="relative">
                <div className="absolute z-30 top-0 left-[170px] w-[380px] bg-surface-2 border border-border-strong rounded-lg shadow-2xl p-4 text-xs">
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-medium text-body">字段类型说明</span>
                    <button onClick={() => setShowTypeHelp(false)} className="text-faint hover:text-body"><i className="fas fa-times"></i></button>
                  </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-[10px] text-muted mb-1">类型标识（唯一ID，不可改）</label><input value={editCmd.cmd} disabled className="w-full px-2 py-1.5 bg-surface-2 border border-border rounded text-faint text-xs font-mono"/></div>
              <div><label className="block text-[10px] text-muted mb-1">中文显示名</label><input value={editCmd.label||''} onChange={e=>setEditCmd({...editCmd,label:e.target.value})} className="w-full px-2 py-1.5 bg-bg border border-border-strong rounded text-body text-xs outline-none focus:border-blue-500"/></div>
              <div><label className="block text-[10px] text-muted mb-1">分类</label><select value={editCmd.category||''} onChange={e=>setEditCmd({...editCmd,category:e.target.value})} className="w-full px-2 py-1.5 bg-bg border border-border-strong rounded text-body text-xs outline-none focus:border-blue-500">{categories.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
              <div><label className="block text-[10px] text-muted mb-1">说明（悬停提示）</label><textarea value={editCmd.description||''} onChange={e=>setEditCmd({...editCmd,description:e.target.value})} rows={2} className="w-full px-2 py-1.5 bg-bg border border-border-strong rounded text-body text-xs outline-none focus:border-blue-500 resize-none" placeholder="描述该指令的功能和使用场景"/></div>
            </div>
          </div>
        </div>
