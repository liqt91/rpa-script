...
export default function AIConfigPage() {
  ...
  if (loading) return <div className="p-6 text-faint"><i className="fas fa-circle-notch fa-spin mr-2" />加载中...</div>;
    ...
    <div className="flex h-full max-h-full overflow-hidden">
      ...
      <div className="w-52 bg-bg border-r border-gray-700 flex flex-col shrink-0">
        ...
        <nav className="flex-1 py-1 overflow-y-auto">
          ...
          <div className="px-4 pt-3 pb-1 text-[10px] text-muted uppercase tracking-wider">场景 Prompt</div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {message && (
          <div className={`mx-5 mt-3 p-3 rounded text-sm shrink-0 ${message.includes('失败') ? 'bg-danger/25 text-danger border border-danger' : 'bg-green-900/30 text-ok border border-green-700'}`}>{message}</div>
        )}
        {nav === 'model' ? <ModelConfig config={config} setConfig={setConfig} />
          ...
          : <div className="flex-1 flex items-center justify-center text-muted text-sm">未找到场景配置</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="px-5 py-2.5 border-b border-gray-700 bg-bg shrink-0 flex items-center gap-3">
        ...
        <code className="text-[10px] text-muted bg-surface-2 px-1.5 py-0.5 rounded">{scenario.id}</code>
        <label className="flex items-center gap-1.5 text-xs text-faint ml-auto">
      ...
      <div className="flex-1 overflow-y-auto p-5">
        <div className="flex flex-col gap-4" style={{ minHeight: 'calc(100vh - 180px)' }}>
          ...
          <div className="flex-1 flex flex-col min-h-[300px]">
            <span className="text-xs text-muted mb-1.5">Prompt 模板</span>
            <textarea value={scenario.prompt}
              onChange={e => updateScenario(scenario.id, 'prompt', e.target.value)}
              className="flex-1 w-full px-4 py-3 bg-surface-2 border border-border-strong rounded text-sm text-body font-mono outline-none focus:border-blue-500 resize-none"
              ...
              <div className="mt-2.5 space-y-2">
                {meta.vars.map(v => (
                  <div key={v.v} className="flex gap-3">
                    <code className="text-xs text-ok bg-bg px-2 py-0.5 rounded shrink-0 font-mono">{v.v}</code>
                    <span className="text-xs text-faint leading-relaxed">{v.d}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="border-t border-gray-700 pt-4 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleTest} disabled={testing || !scenario.enabled}
                className="text-xs px-3 py-1.5 rounded bg-ok/25 text-ok hover:bg-ok/40 disabled:opacity-50">
            ...
            {(testPrompt || testResult) && (
              <div className="mt-3 grid grid-cols-2 gap-3" style={{ height: '350px' }}>
                <div className="flex flex-col min-h-0">
                  <span className="text-[10px] text-muted mb-1 shrink-0">填充后的 Prompt</span>
                  <pre className="flex-1 p-3 bg-bg rounded text-[10px] text-faint font-mono overflow-auto whitespace-pre-wrap">{testPrompt || '(empty)'}</pre>
                </div>
                <div className="flex flex-col min-h-0">
                  <span className="text-[10px] text-muted mb-1 shrink-0">AI 返回结果</span>
                  <pre className="flex-1 p-3 bg-bg rounded text-[10px] text-ok font-mono overflow-auto whitespace-pre-wrap">{testResult || '(empty)'}</pre>
                </div>
              </div>
            )}
          </div>
