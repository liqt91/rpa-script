...
export default function CommandEditor() {
  ...
  const [sandboxContext, setSandboxContext] = useState({ windows: [], isAnyOnline: false });
  const [testClientId, setTestClientId] = useState('');
  const [testCollapsed, setTestCollapsed] = useState(false);
  const [testMode, setTestMode] = useState('quick'); // quick | flow
  ...
  async function saveCategoriesJson() {
    try {
      const data = JSON.parse(categoriesJson);
      await api.saveCategoriesJson(data);
      await loadCategories();
      setStatus('分类已保存');
    } catch (e) { setError('保存分类失败: ' + e.message); }
  }
  ...
  async function loadSandboxContext() {
    try {
      const data = await api.getSandboxContext();
      setSandboxContext(data);
      if (data.windows.length > 0 && !testClientId) {
        setTestClientId(data.windows[0].clientId);
      }
    } catch (e) { /* ignore */ }
  }

  async function loadTestTemplates(typeName) {
    try {
      const data = await api.getTestTemplates(typeName);
  ...
  // ── sandbox test ──
  function updateTestParam(name, value) {
  ...
  function fillExampleParams() {
    ...
    for (const p of form.params) {
      if (p.default !== undefined && p.default !== null) {
        examples[p.name] = p.default;
      } else if (p.type === 'select' && p.options?.length) {
        examples[p.name] = p.options[0].value;
      } else if (p.type === 'number') {
        examples[p.name] = 1;
      } else if (p.type === 'boolean') {
        examples[p.name] = true;
      } else if (p.type === 'string' || p.type === 'text') {
        examples[p.name] = p.name === 'url' ? 'https://example.com' : 'test';
      } else {
        examples[p.name] = '';
      }
    }
    setTestParams(examples);
  }

  async function runTest() {
    if (!form || (!isBackend && !isExtension)) return;
    if (isExtension && !testClientId) {
      setError('extension 指令需要选择浏览器窗口才能测试');
      return;
    }
    setTestLoading(true);
    setTestResult(null);
    setError('');
    try {
      const res = await api.testCommand(form.cmd, testParams, {}, testClientId || null);
      setTestResult(res);
      if (res.success) {
        setStatus(`测试通过 (${res.elapsed}s)`);
      } else {
        setError(res.error || '测试失败');
      }
    } catch (e) {
      setTestResult({ success: false, error: e.message });
      setError('测试失败: ' + e.message);
    } finally {
  ...
  function applyTemplate(tpl) {
    const nodes = tpl.nodes.map(n => ({
      ...
      extra: Object.fromEntries(
        Object.entries(n.extra || {}).map(([k, v]) => {
          if (typeof v === 'string') {
            let val = v;
            for (const [pk, pv] of Object.entries(tpl.params || {})) {
    ...
    setFlowNodes(JSON.stringify(nodes, null, 2));
  }

  async function runTestFlow() {
    if (!form || !flowNodes.trim()) return;
    setFlowLoading(true);
    setFlowResult(null);
    setError('');
    try {
      const nodes = JSON.parse(flowNodes);
      const res = await api.runTestFlow(form.cmd, nodes, {}, testClientId || null);
      setFlowResult(res);
      if (res.success) {
        setStatus('测试流程执行成功');
      } else {
        setError(res.error || '测试流程执行失败');
      }
    } catch (e) {
      setFlowResult({ success: false, error: e.message });
      setError('测试流程失败: ' + e.message);
    } finally {
  ...
  async function generateFlowWithAI() {
    if (!form || !aiFlowDesc.trim()) return;
    setAiFlowLoading(true);
    setError('');
    try {
      const res = await api.generateTestFlow(form.cmd, aiFlowDesc, null);
      const code = res.code || '';
      // Try to parse JSON array from code
      try {
        const nodes = JSON.parse(code);
        setFlowNodes(JSON.stringify(nodes, null, 2));
        setStatus('AI 生成测试流程成功');
      } catch (e2) {
        setFlowNodes(code);
        setError('AI 返回的内容不是合法 JSON，请手动修正');
      }
    } catch (e) {
      ...
      <div className="w-56 bg-[#0f172a] border-r border-gray-700 flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
          ...
          <button
            ...
            className={`w-full text-left px-2 py-1.5 rounded text-xs transition-colors flex items-center gap-2 ${
              selected && selected._type === '__categories__' ? 'bg-purple-600/30 text-purple-200' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'
            }`}>
        ...
        {selected && selected._type === '__type_registry__' ? (
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="px-4 py-2 border-b border-gray-700 flex items-center justify-between bg-[#0f172a] shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-200">分类管理</span>
                <code className="text-[10px] text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded">src/runtime/commands/types/categories.json</code>
              </div>
              <div className="flex items-center gap-2">
                {status && <span className={`text-xs ${status.includes('失败') || error ? 'text-red-400' : 'text-green-400'}`}>{status}</span>}
                {error && <span className="text-xs text-red-400">{error}</span>}
                <button onClick={saveCategoriesJson}
                  className="text-xs px-3 py-1 rounded bg-blue-600 text-white hover:bg-blue-500">
                  <i className="fas fa-save mr-1"></i>保存
                </button>
              </div>
          ...
          <>
            ...
            <div className="flex-1 flex min-h-0">
              ...
              {/* Col 5: 测试 */}
              <div className="flex-1 flex flex-col min-w-0 border-l border-gray-700">
                <div className="px-3 py-1.5 border-b border-gray-700 flex items-center justify-between bg-[#0f172a] shrink-0">
                  <span className="text-[10px] font-medium text-gray-400">测试</span>
                  <button onClick={() => setTestCollapsed(!testCollapsed)}
                    className="text-gray-500 hover:text-gray-300 text-[10px]">
                    <i className={`fas fa-chevron-${testCollapsed ? 'down' : 'up'}`}></i>
                  </button>
                </div>
                {testCollapsed ? null : !isBackend && !isExtension ? (
                  <div className="flex-1 flex items-center justify-center text-gray-500 text-xs px-3 text-center">
                    仅本地端操作指令和扩展端执行指令支持测试
                  </div>
                ) : (
                  <>
                    {/* Mode tabs */}
                    <div className="px-3 py-1.5 border-b border-gray-700/50 bg-[#0a0f1a] shrink-0 flex items-center gap-2">
                      <button onClick={() => setTestMode('quick')}
                        className={`text-[10px] px-2 py-1 rounded ${testMode === 'quick' ? 'bg-blue-600/30 text-blue-300' : 'text-gray-400 hover:text-gray-200'}`}>
                        快速预览
                      </button>
                      <button onClick={() => setTestMode('flow')}
                        className={`text-[10px] px-2 py-1 rounded ${testMode === 'flow' ? 'bg-blue-600/30 text-blue-300' : 'text-gray-400 hover:text-gray-200'}`}>
                        测试流程
                      </button>
                    </div>
                    ...
                    {testMode === 'quick' && (
                      <>
                        <div className="px-3 py-2 border-b border-gray-700/50 bg-[#0a0f1a] shrink-0 space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] text-gray-400">参数</span>
                            <div className="flex items-center gap-1.5">
                              <button onClick={fillExampleParams}
                              ...
                              <button onClick={runTest} disabled={testLoading}
                                className="text-[10px] px-2 py-1 rounded bg-green-700/60 text-green-200 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed">
                                {testLoading ? <i className="fas fa-circle-notch fa-spin"></i> : '运行'}
                              </button>
                            </div>
                          </div>
                          <div>
                            <div className={labelCls}>浏览器窗口（可选）</div>
                            <select
                              value={testClientId}
                              onChange={e => setTestClientId(e.target.value)}
                              className={selectCls}
                            >
                              <option value="">不连接浏览器</option>
                              {sandboxContext.windows.map(w => (
                                <option key={w.clientId} value={w.clientId}>
                                  {w.browser} — {w.title || w.url || w.clientId}
                                </option>
                              ))}
                            </select>
                            {!sandboxContext.isAnyOnline && (
                              <div className="text-[10px] text-yellow-500 mt-1">
                                当前无浏览器连接，仅可测试纯 backend 指令
                              </div>
                            )}
                          </div>
                        </div>
                        <div className="flex-1 overflow-y-auto p-3 space-y-3">
                          {(form.params || []).map((p, i) => (
                            <div key={i}>
                              <div className={labelCls}>{p.label || p.name} <span className="text-gray-600">({p.name})</span></div>
                              {p.type === 'select' ? (
                                <select
                                  value={testParams[p.name] ?? p.default ?? ''}
                                  onChange={e => updateTestParam(p.name, e.target.value)}
                                  className={selectCls}
                                >
                                  {(p.options || []).map(opt => (
                                ...
                                <label className="flex items-center gap-1.5 text-xs text-gray-300">
                                  <input
                                    type="checkbox"
                                    checked={testParams[p.name] ?? p.default ?? false}
                                    onChange={e => updateTestParam(p.name, e.target.checked)}
                                    className="accent-blue-500"
                                  />
                                  {testParams[p.name] ? 'true' : 'false'}
                                </label>
                              ) : p.type === 'number' ? (
                                <input
                                  type="number"
                                  value={testParams[p.name] ?? p.default ?? ''}
                                  onChange={e => updateTestParam(p.name, Number(e.target.value))}
                                  className={inputCls}
                                />
                              ) : (
                                <input
                          ...
                          {testResult && (
                            <div className="mt-3 pt-3 border-t border-gray-700/50">
                              <div className={labelCls}>执行结果</div>
                              <pre className="text-[10px] font-mono bg-[#0a0f1a] p-2 rounded overflow-auto max-h-64 text-gray-300 whitespace-pre-wrap">
                      ...
                      <div className="flex-1 flex flex-col min-h-0">
                        <div className="px-3 py-2 border-b border-gray-700/50 bg-[#0a0f1a] shrink-0 space-y-2">
                          ...
                          {testTemplates.length > 0 && (
                            <div>
                              <div className={labelCls}>测试模板</div>
                              <div className="space-y-1">
                                {testTemplates.map((t, i) => (
                                  <button key={i} onClick={() => applyTemplate(t)}
                                    className="w-full text-left text-[10px] px-2 py-1.5 rounded bg-gray-800/50 text-gray-300 hover:bg-gray-700/50">
                                    <div className="font-medium">{t.name}</div>
                                    <div className="text-gray-500 text-[9px]">{t.description}</div>
                                  </button>
                                ))}
                              </div>
                            </div>
                          ...
                          {/* AI generate */}
                          <div>
                            <div className={labelCls}>AI 生成测试流程</div>
                            <div className="flex gap-1.5">
                              <input
                                value={aiFlowDesc}
                                onChange={e => setAiFlowDesc(e.target.value)}
                                placeholder="描述测试目标，如：打开百度，点击搜索按钮"
                                className="flex-1 px-2 py-1 bg-[#0f172a] border border-gray-600 rounded text-gray-200 text-[10px] outline-none focus:border-blue-500"
                              />
                              <button onClick={generateFlowWithAI} disabled={aiFlowLoading || !aiFlowDesc.trim()}
                                className="text-[10px] px-2 py-1 rounded bg-purple-600/80 text-white hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed shrink-0">
                                {aiFlowLoading ? <i className="fas fa-circle-notch fa-spin"></i> : '生成'}
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="flex items-end gap-1.5">
                            <div className="flex-1">
                              <div className={labelCls}>浏览器窗口</div>
                              <select
                                value={testClientId}
                                onChange={e => setTestClientId(e.target.value)}
                                className={selectCls}
                            ...
                            <button onClick={runTestFlow} disabled={flowLoading || !flowNodes.trim()}
                              className="text-[10px] px-2 py-1 rounded bg-green-700/60 text-green-200 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed shrink-0">
                              {flowLoading ? <i className="fas fa-circle-notch fa-spin"></i> : '运行流程'}
                            </button>
                          </div>
                        </div>

                        <div className="flex-1 flex flex-col min-h-0">
                          <div className="px-3 py-1.5 border-b border-gray-700/50 text-[10px] text-gray-500 bg-[#0a0f1a] shrink-0">
                            流程节点（JSON）
                          </div>
                          <textarea
                            value={flowNodes}
                            onChange={e => setFlowNodes(e.target.value)}
                            className="flex-1 p-2 text-[10px] font-mono bg-[#0a0f1a] text-gray-300 outline-none resize-none"
                            spellCheck={false}
                            placeholder='[{"cmd": "launchBrowser", "order": 1, "extra": {...}}]'
                          />
                          {flowResult && (
                            <div className="px-3 py-2 border-t border-gray-700/50 shrink-0 max-h-48 overflow-y-auto">
                              <div className={labelCls}>执行结果</div>
                              <pre className="text-[10px] font-mono bg-[#0a0f1a] p-2 rounded text-gray-300 whitespace-pre-wrap">
