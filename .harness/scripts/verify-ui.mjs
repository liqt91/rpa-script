...
async function runMockValidation(runDir) {
  const screenshotPath = join(runDir, 'screenshots', 'home.txt');
  await mkdir(join(runDir, 'screenshots'), { recursive: true });
  await writeFile(screenshotPath, 'mock screenshot placeholder\n');
  return {
    passed: true,
    url: opts.url || 'mock://ui',
    durationMs: 1,
    checks: [
      { name: 'page-load', passed: true, detail: 'mock page loaded' },
      { name: 'console-errors', passed: true, detail: '0 errors' },
      { name: 'network-failures', passed: true, detail: '0 failed requests' },
      { name: 'screenshot', passed: true, detail: rel(screenshotPath) },
    ],
    console: [],
    networkFailures: [],
    screenshots: [rel(screenshotPath)],
  };
}
...
<p>URL: ${escapeHtml(summary.url)} · Duration: ${summary.durationMs}ms · Run: ${escapeHtml(summary.runId)}</p>
...
async function main() {
  ...
  console.log(JSON.stringify({ passed: summary.passed, runId, artifactDir: summary.artifactDir, report: rel(join(runDir, 'report.html')), screenshots: summary.screenshots }, null, 2));
