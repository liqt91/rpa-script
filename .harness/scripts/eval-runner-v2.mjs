...
async function loadTasks(opts) {
  ...
  return Promise.all(files.map(async f => ({ ...JSON.parse(await readFile(join(dir, f), 'utf8')), _file: f })));
}
...
async function runEval(opts) {
  ...
  for (const task of tasks) {
    ...
    const grades = [gradeProcess(task, tools), gradeStyle(task, files), gradeRequiredFiles(task, files)].filter(Boolean);
