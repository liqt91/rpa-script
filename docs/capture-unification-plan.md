...
> **现状 vs 目标的差距（2026-08-19 验证）——「框选绘制统一性」未完成**
>
> 不变量 3 要求"高亮统一由遮罩绘制；hover 与点击走同一 resolver 链（所见即所得）"。
> 但当前实现（`overlay_mask.py` 主循环）仍是**两套绘制**，切换点 = `overlay_mask.py:477`
> `if browser and _extension_online(...)` → `mask.hide()` + `_capture_via_extension`：
>
> | 通道 | 框选由谁画 |
> |---|---|
> | 纯 UIA（扩展离线 / 非浏览器内容框） | **我们的全屏遮罩**（蓝框 + 光标孔 + 洋葱高亮） |
> | 扩展托管（扩展在线 + 光标在浏览器内容框） | **扩展 content 脚本页面内高亮**（遮罩 `mask.hide()` 隐藏） |
>
> 后果：插件启用后网页捕获的框选换成页内高亮，与纯 UIA 的遮罩蓝框视觉两套、切换时闪烁/错位，
> 正是方案要消除的"模式状态机"残留（`mask.hide()`/页内 canvas 即是网页模式的影子）。
>
> **按方案收敛方向（P2）**：扩展只做 `resolveAtPoint` 纯查询（返回元素/选择器，不画高亮），
> 高亮统一回遮罩绘制；`mask.hide()`/`_capture_via_extension` 的页内高亮、`session_holder`/
> `_cursor_in_viewport` 网页会话双路路由整套删除，主循环按帧路由 resolver、始终画遮罩。
> 用户已确认捕获通道取向：**扩展优先、UIA 兑底**（方案原旨）。
