...
| **Background handler** | `extension/background_handlers/` | 不编译，`background.js` 按需 `importScripts` 分发 | `chrome.windows`, `chrome.tabs` 等扩展后台 API |
...
│   │   ├── inputElement.js
│   │   └── waitForElement.js
│   ├── background_handlers/               ← Background handler（background.js 分发）
│   │   ├── _load_all.js                   ← 聚合 importScripts
│   │   └── launchBrowser.js
│   ├── background.js                      ← 纯路由器（不含业务逻辑，不含类型名硬编码）
│   ├── content.js                         ← 编译产物（由 build_content_js.py 生成）
│   ├── finder.js                          ← 元素查找工具
│   └── ...
│
└── scripts/                               ← 构建 & AI 工具
    ├── generate_commands.py               ← JSON → .py/.js 桩代码
    ├── build_content_js.py                ← dom_handlers + content_base → content.js
...
| `handler.function` | extension专用 | JS 函数名（delegate 模式，一行转发） |
| `handler.source` | extension专用 | JS 源文件路径（custom 模式，复制文件） |
...
| kind | 行为 |
|---|---|
| `"extension"` | `generate_commands.py` 生成 Python 桩 + JS handler。`function` → delegate 一行转发；`source` → 复制源文件；都没有 → TODO 桩 |
...
|---|---|---|
| `"extension"` + `function` | 生成注册桩（已有则 KEEP） | 生成 delegate 一行转发（已有则 KEEP） |
...
```js
// background.js — 纯路由器
// 1. 启动时 importScripts 加载所有 background_handlers
importScripts('background_handlers/_load_all.js');

// 2. 收到 executeStep 时按 type 分发
async _handleExecuteStep(step) {
  const handler = _backgroundHandlers[step.type];
  if (handler) {
    return await handler(step, this);
  }
...
2. `POST /api/commands/definitions/build` → 生成 Python 桩
3. 手动创建 `extension/background_handlers/<type>.js`，调用 `registerBackgroundHandler()`
4. 更新 `extension/background_handlers/_load_all.js` 添加 `importScripts`
...
| `clickElement` | extension (DOM) | Python 桩 + delegate JS（function: "doClick"） |
| `inputElement` | extension (DOM) | Python 桩 + delegate JS（function: "doInput"） |
| `waitForElement` | extension (DOM) | Python 桩 + custom JS（source 文件） |
