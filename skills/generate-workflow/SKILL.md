...
1. **只能使用 `commands/*.json` 中已存在的指令**。禁止在生成过程中"顺手发明/新建"指令。
2. 生成前先 `rpa_commands(side=editor)` 盘一遍现有指令能力面板，确认能覆盖需求；不要凭空想象指令名。
3. 某一步骤**现有指令确实无法表达**（功能缺失）才考虑新建，且必须：
   - 明确说出"缺哪个能力 / 哪一步"（不是"造个更贴描述的指令"）；
   - 用 `rpa_new_command`（或 `command_builder.py`）走**正规新建流程**；
   - **先向用户说明并征得确认**，再执行新建。
4. 优先**组合现有指令**：复杂需求拆成现有基础指令能表达的最小步骤；缺的若是"参数/分支/细小变体"，先看能否在现有指令上复用/组合，而非另起炉灶。
5. 若现有指令本身有缺陷，**优先修正已有指令**，而不是新建一个"平行指令"（避免指令膨胀）。
6. 新建指令有真实成本：新增 JSON 定义 + handler + 可能需重载扩展/重启后端；**不是工作流里某个节点的选择**。
7. 生成的工作流**默认应完全由现有指令构成**；任何"新建指令"都必须是用户确认后的例外。

> 判定顺序：**复用现有 → 组合现有 → 修正现有 → 确实缺功能 → 说明并确认 → 才新建**。
